...
* File Name:   main.c
*
* Description: Multi phase synchronous voltage control mode buck regulator
...
void hardware_init(void)
{
    ...
    /* Connect CSG slice 1 comparator output to the BUCK1 HW protection trigger */
    HPPASS_config.trigLevel[0].compMsk = CY_HPPASS_INTR_CSG_1_CMP;

    /* Connect CSG slice 3 comparator output to the BUCK2 HW protection trigger */
    HPPASS_config.trigLevel[1].compMsk = CY_HPPASS_INTR_CSG_3_CMP;
    ...
    if (CY_RSLT_SUCCESS != BUCK1_init())
    {
        CY_ASSERT(0);
    }

    /* Sets the DAC value for buck converter 1 */
    Cy_HPPASS_DAC_SetValue(BUCK_1_CSG_IND_CUR_SLICE_IDX,930);

    /* Sets the DAC value for buck converter 2 */
    Cy_HPPASS_DAC_SetValue(BUCK_2_CSG_IND_CUR_SLICE_IDX,930);

    /*Start the DAC for buck converter 1 */
    Cy_HPPASS_DAC_Start(BUCK_1_CSG_IND_CUR_SLICE_IDX, CY_HPPASS_DAC_FW);

    /*Start the DAC for buck converter 2 */
    Cy_HPPASS_DAC_Start(BUCK_2_CSG_IND_CUR_SLICE_IDX, CY_HPPASS_DAC_FW);

    /* Start the HPPASS */
    HPPASS_start();

    /* Clear the comparator interrupt */
    Cy_HPPASS_Comp_ClearInterrupt((CY_HPPASS_INTR_CSG_1_CMP | CY_HPPASS_INTR_CSG_3_CMP));

    /* Configures the comparator interrupt */
    Cy_HPPASS_Comp_SetInterruptMask((CY_HPPASS_INTR_CSG_1_CMP | CY_HPPASS_INTR_CSG_3_CMP));
}
...
void soft_start_prot_intr_handler(void)  // rename
{
    ...
    {
        /* Stops the counter when both counters are reached the reference value. */
...
void button_press_intr_handler(void)
{
    ...
    switch (buck_state)
    {
        ...
        {
            /* Resets variables used for protection of converter 1. */
            ...
            /* Starts the buck converters simultaneously. */
            result = BUCK1_start();
            if (result != CY_RSLT_SUCCESS)
            {
                CY_ASSERT(0);
        ...
        case Ifx_BUCK_STATE_TEST: /* Button press when state is Test. State will switch to Idle state. */
        {
            ...
            /* Stops the DAC for buck converter 1 */
            Cy_HPPASS_DAC_Stop(BUCK_1_CSG_IND_CUR_SLICE_IDX);

            /* Stops the DAC for buck converter 2 */
            Cy_HPPASS_DAC_Stop(BUCK_2_CSG_IND_CUR_SLICE_IDX);
...
int main(void)
{
    ...
    printf("\x1b[2J\x1b[;H");
    printf("\r\n---------------------------------------------------------------------------------------------------------------------------------------------------"
           "\r\nThis code example demonstrates the voltage control mode multi-phase buck converter implementation on the KIT_PSC3M5_DP1."
           "\r\n "
           ...
           "\r\n"
           "\r\nFor more information, see the README.md of the mtb-example-ce241297-vcm-buck-multi-phase code example."
           "\r\n---------------------------------------------------------------------------------------------------------------------------------------------------\r\n"
